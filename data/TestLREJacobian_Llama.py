#This script was used to produce test results for Llama.
...
RESULTS_FILE = 'results/42Thursday_Cuda0_All_Llama_4_16.txt'
...
def weight_and_bias(start: int, relation: str) -> dict:
    wdir = f"llama_{start}_31_approx/{relation}"
    ...
    weight_name = f's_o_weight_{start}_31'
    bias_name = f's_o_bias_{start}_31'
...
#     wdir = f"llama_{start}_27_approx_W2V/{relation}"
#     ignored_names = ['.ipynb_checkpoints']
#     try:
#         _ = os.listdir(wdir)
#     except:
...
for beta in [7]:  
    for i in range(0, N_TRIALS):
        for relation in relations:
            ...
            wb_dicts = [weight_and_bias(i, relation.name) for i in range(4,17)]
            #offsets = [make_offset_dict(i, relation.name) for i in range(8,17)]
            ...
            for start in range(4,17):
                ...
                zip(range(0,42), test_samples, clozed_answers, clozed_prompts, preds_lm):
            ...
            logger.info(f'{relation.name},{beta},{best_layer},{top_reg_correct},{lm_at_top_reg_correct},{top_no_bias_correct},{lm_at_top_no_bias_correct},{top_no_weight_correct},{lm_at_top_no_weight_correct},{top_w2v_correct},{lm_at_top_w2v_correct}')
