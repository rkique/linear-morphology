...
def order_1_approx(
    *,
    ...
    for j in range(h_layer, z_layer):
        ...
        directory = Path(f'kapprox/{subject}')
        
        if not directory.exists():
            directory.mkdir(parents=True, exist_ok=True)

        torch.save(s_o_weight, f'kapprox/{subject}/s_o_weight_h_{j}.pt')
        torch.save(s_s_weight, f'kapprox/{subject}/s_s_weight_h_{j}.pt')
        torch.save(o_o_weight, f'kapprox/{subject}/o_o_weight_h_{j}.pt')

        torch.save(s_o_bias, f'kapprox/{subject}/s_o_bias_h_{j}.pt')
        torch.save(s_s_bias, f'kapprox/{subject}/s_s_bias_h_{j}.pt')
        torch.save(o_o_bias, f'kapprox/{subject}/o_o_bias_h_{j}.pt')

        torch.save(s_j, f'kapprox/{subject}/hs_h_{j}.pt')
        torch.save(o_j, f'kapprox/{subject}/hs_o_{j}.pt')
        torch.save(s_j1, f'kapprox/{subject}/hs_h_{j+1}.pt')
        torch.save(o_j1, f'kapprox/{subject}/hs_o_{j+1}.pt')
