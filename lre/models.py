...
import lre.env_utils as env_utils
from lre.lretyping import Device, Layer, Model, ModelInput, Tokenizer
...
#This sets padding size for a ModelAndTokenizer.
@contextmanager
...
def load_model(
    ...
    #... the AutoModelForCausalLM adds an additional linear network layer on top of the model. As an example if you’re training a GPT-2 model to auto-complete sentences, the added layer maps whatever the GPT-2 model embedding output is to the real next word. Putting a phrase to be autocompleted through a model loaded with just AutoModel would output just the dense math representation generated by the base model. 
    #call to transformers.AutoModelForCausalLM
